...
public sealed partial class DownloadPageViewModel : ObservableObject
{
    ...
    public bool HasNoLoaderVersions => !IsLoadingLoaderVersions
        ...
        && SelectedLoaderOption?.Kind is LoaderKind.Fabric
    ...
    public bool ShouldShowFabricVersionSelector => IsInstanceOptionsStep && SelectedLoaderOption?.Kind is LoaderKind.Fabric;
    ...
    public string LoaderVersionPlaceholderText => IsLoadingLoaderVersions
        ? string.Format(Strings.Status_LoadingLoaderVersionsFormat, Strings.Download_FabricLoaderTitle)
        : !string.IsNullOrWhiteSpace(LoaderVersionLoadError)
            ? Strings.Download_LoaderVersionLoadFailedShort
            : HasNoLoaderVersions
                ? Strings.Download_FabricLoaderVersionEmpty
    ...
    private bool CanInstall()
    {
        var canInstallWithSelectedLoader = SelectedLoaderOption?.Kind switch
        {
            ...
            LoaderKind.Fabric => !IsLoadingLoaderVersions
                && string.IsNullOrWhiteSpace(LoaderVersionLoadError)
    ...
    partial void OnSelectedLoaderOptionChanged(DownloadLoaderOption? value)
    {
        ...
        OnPropertyChanged(nameof(HasNoLoaderVersions));
        OnPropertyChanged(nameof(ShouldShowFabricVersionSelector));

        if (value?.Kind is not null and not LoaderKind.Vanilla and not LoaderKind.Fabric)
        {
            InstallError = string.Empty;
    ...
    {
        if (loaderOption?.Kind is not LoaderKind.Fabric || SelectedMinecraftVersion is null || CurrentStep is not DownloadPageStep.InstanceOptions)
        {
            ResetLoaderVersionState();
            return;
        }

        if (!loaderProviders.TryGetValue(LoaderKind.Fabric, out var provider) || !provider.IsImplemented)
        {
            ResetLoaderVersionState();
            OnPropertyChanged(nameof(LoaderVersionPlaceholderText));
            NotifyInstallStateChanged();
        ...
        try
        {
            var loaderVersions = await provider.GetLoaderVersionsAsync(SelectedMinecraftVersion.Name);
        ...
        catch (Exception)
        {
            ...
            LoaderVersionLoadError = Strings.Status_FabricLoaderVersionsLoadFailed;
        }
        finally
        {
            if (IsLoaderVersionRequestCurrent(requestVersion, loaderOption))
    ...
    {
        if (SelectedLoaderOption?.Kind is LoaderKind.Fabric && !string.IsNullOrWhiteSpace(SelectedLoaderVersion?.Version))
            return $"{SelectedLoaderOption.Title} {SelectedLoaderVersion.Version}";
