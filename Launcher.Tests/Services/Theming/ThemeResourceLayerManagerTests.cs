...
public sealed class ThemeResourceLayerManagerTests
{
    ...
    [Fact]
    public void ImageLayerAloneEnablesHomeForegroundTextShadow()
    {
        RunOnStaThread(() =>
        {
            var manager = new ThemeResourceLayerManager(source =>
            {
                ...
                if (source.EndsWith(
                        ...
                        StringComparison.Ordinal))
                {
                    dictionary["Effect.Home.ForegroundTextShadow"] = new DropShadowEffect();
                }
            ...
            {
                ["Effect.Home.ForegroundTextShadow"] = null
            };
            resources.MergedDictionaries.Add(shared);
            var target = new TextBlock { Resources = resources };
            target.SetResourceReference(
            ...
            manager.ApplyLayers(
                ...
                imageBackgroundEnabled: true,
                imageControlBlurEnabled: false);
            Assert.IsType<DropShadowEffect>(target.Effect);
    ...
    [InlineData(42, 0.42d)]
    [InlineData(120, 1d)]
    public void ApplyPageBackgroundOpacityClampsAndPublishesScalarResource(
        int opacityPercent,
        double expected)
    {
        var resources = new ResourceDictionary();

        ThemeResourceLayerManager.ApplyPageBackgroundOpacity(resources, opacityPercent);

        Assert.Equal(expected, resources[ThemeResourceLayerManager.PageBackgroundOpacityResourceKey]);
    }
